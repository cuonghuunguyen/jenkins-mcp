...
import { resolveTimeoutMs } from "./wait.js";
...
/** Fallback bound when the caller passes a non-number (see `resolveTimeoutMs`). */
...
export async function pollQueueItem(
  ...
  // `NaN` would remove the `>= timeoutMs` exit entirely (every comparison
  // against NaN is false), which is exactly what `jenkins build trigger
  // --timeout abc` produces via yargs. Guarded here, not at the call site.
  const bound = resolveTimeoutMs(timeoutMs, DEFAULT_QUEUE_TIMEOUT_MS);
